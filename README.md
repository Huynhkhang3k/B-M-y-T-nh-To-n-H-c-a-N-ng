...
<html lang="vi">
  ...
  <body>
    <!-- Nút chuyển đổi duy nhất giữa 2 máy tính (chỉ hiện trên Máy Tính Toán Học) -->
    <button class="toggleButton" id="showTriangleCalc">
      Chuyển sang Máy Tính Tam Giác Vuông
    </button>
    ...
    <div class="container" id="triangleCalc">
      <!-- Chỉ một nút "Trở về" -->
      <button class="toggleButton" id="showOriginalCalc">
        Trở về Máy Tính Toán Học
      </button>
      <h2>Máy Tính Tam Giác Vuông</h2>
      <p style="font-family: 'Lobster', cursive">
        Nhập ít nhất 2 giá trị. Tam giác được vẽ với góc vuông ở góc dưới bên
        trái.
      </p>
      <!-- Nhập các cạnh -->
...
<strong>Kết quả của biểu thức "<em>${expr}</em>" là:</strong> <span style="font-size:1.8em;">${result}</span>.`;
    ...
    <!-- Mã chuyển đổi giữa 2 máy tính -->
    <script>
      const originalCalc = document.getElementById("originalCalc");
      const triangleCalc = document.getElementById("triangleCalc");
      const showTriangleBtn = document.getElementById("showTriangleCalc");
      const showOriginalBtn = document.getElementById("showOriginalCalc");
